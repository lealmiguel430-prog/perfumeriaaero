    ...
    <main className="flex-grow flex items-center justify-center py-20 px-4">
      <div className="w-full max-w-md bg-[#1A2220] border border-gold/10 p-8 md:p-10 shadow-2xl relative overflow-hidden">
        ...
        <div className="text-center mb-10">
          <h1 className="text-3xl font-serif text-cream mb-2">Crear Cuenta</h1>
          <p className="text-gray-400 text-sm">Únete a AERO Perfumería</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-2">
            <label htmlFor="name" className="text-xs uppercase tracking-widest text-gold/80 font-bold block">
              Nombre Completo
            </label>
          ...
          <div className="flex items-center text-xs text-gray-400">
            <input type="checkbox" required className="mr-2 rounded border-gray-600 bg-transparent text-gold focus:ring-0" />
            <span>
              Acepto los <Link to="/terminos" className="text-gold underline">términos y condiciones</Link>
            </span>
          </div>
        ...
        <div className="mt-8 pt-8 border-t border-gold/10 text-center">
          <p className="text-gray-400 text-sm">
            ...
            <Link to={`/login?redirect=${redirectPath}`} className="text-gold font-bold hover:text-gold-light hover:underline">
              Inicia sesión aquí
            </Link>
          </p>
        </div>
